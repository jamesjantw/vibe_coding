# Vibe Coding - BMad Method V6 Alpha 开发指南

Vibe Coding 是一個展示如何在 Kilo Code 中使用 BMad Method V6 Alpha 進行结构化敏捷开发的范例项目。

## 🌐 语言选择 / Language Selection

- [简体中文 (Traditional Chinese)](README.md)
...
**适用 IDE**: Kilo Code, VS Code, Cursor, Windsurf, Claude Code 等

## 🚀 为什么選擇 BMad Method V6？

**BMad Method V6 Alpha** 是一個革命性的 AI 驱动开发框架，结合了：
- 🤖 **智慧代理协作** - 多角色 AI 代理协同工作
- 📋 **结构化工作流程系統** - 全新的互动式工作流程（Workflow）取代旧版任务（Tasks）
- 🎯 **质量保证** - 內建测试策略和质量门槛
- 🔄 **持续改进** - 基於反馈的迭代优化
- 🎨 **规模自适应** - 自動根據项目复杂度调整规划深度

### V6 Alpha 核心新特性

- **工作流程系統** - 使用互动式工作流程取代旧版任务和模板
- **简化指令** - 使用 `*workflow-name` 或自然語言启动工作流程
- **自动追踪** - `workflow-init` 自動建立项目追蹤，`workflow-status` 随时查看进度
- **三种规划轨迹** - Quick Flow、BMad Method、Enterprise Method
- **上下文管理** - 建议每個工作流程使用新對話，避免上下文限制

### 核心优势

- **提升开发效率** - 减少重复工作，专注于創造性任务
- **确保质量一致性** - 标准化的流程和检查点
- **降低沟通成本** - AI 代理处理例行沟通
...
| 代理 | 角色 | 主要工作流程 | 使用時機 |
|------|------|------------|----------|
| **Analyst** | 分析師 | `*workflow-init`, `*workflow-status`, `*brainstorm-project`, `*research` | 项目初始化、进度追蹤、研究和腦力激盪 |
...
| **Dev** | 开发者 | `*dev-story`, `*code-review` | 實作功能、程式碼審查 |
| **UX Designer** | UX 設計師 | `*ux` | 建立 UX 設計文件 |
| **TEA** | 測試架構師 | 测试策略和质量保证 | 质量保证和测试策略 |
| **Technical Writer** | 技術文件撰寫 | 文件生成 | 技術文件撰寫 |

### 4. 初始化工作流程

**重要：每個工作流程建议使用新對話，以避免上下文限制**
...
| 工作流程 | 代理 | 說明 |
|---------|------|------|
| `*workflow-init` | Analyst | 初始化项目工作流程追蹤 |
| `*workflow-status` | 任何代理 | 查看當前进度和下一步建议 |

#### 規劃階段（Phase 1-2）

| 工作流程 | 代理 | 適用軌跡 | 說明 |
|---------|------|---------|------|
| `*brainstorm-project` | Analyst | 所有 | 项目腦力激盪（可選） |
| `*research` | Analyst | 所有 | 領域研究（可選） |
| `*product-brief` | Analyst | BMad Method/Enterprise | 產品簡報（可選，僅綠地项目） |
...
| `*ux` | UX Designer | 所有 | UX 設計文件（UI 项目） |

#### 方案階段（Phase 3）

| 工作流程 | 代理 | 適用軌跡 | 說明 |
|---------|------|---------|------|
| `*create-architecture` | Architect | BMad Method/Enterprise | 建立架構文件（Level 3-4 或建议） |
...
| `*story-context` | SM | 建立故事技術上下文（每個故事，建议） |
| `*dev-story` | Dev | 實作故事 |
| `*code-review` | Dev | 程式碼審查（建议） |
...
```mermaid
graph TD
    A[项目開始] --> B[Analyst: *workflow-init]
    B --> C{選擇規劃軌跡}
...
**適用場景：** Level 4 项目（30+ 個故事）
- 企業需求、合規性、多租戶
- BMad Method + 安全架構 + DevOps + 测试策略
- 完整的企業級規劃
...
5. **文件驱动** - 使用 PRD 和架構作為开发指南
6. **定期檢查狀態** - 使用 `workflow-status` 查看进度

### 代理使用建议

- **Analyst**: 项目初始化、进度追蹤、研究和腦力激盪
- **PM**: 需求定義、PRD/技術規格建立、Epic 和故事管理
- **Architect**: 架構設計、方案驗證
- **SM**: 衝刺規劃、故事管理、回顧
- **Dev**: 程式碼實作、程式碼審查
- **UX Designer**: UI/UX 設計文件（UI 项目）
...
- **GitHub**: [回報問題與建议](https://github.com/bmad-code-org/BMAD-METHOD/issues)
- **YouTube**: [BMadCode 頻道](https://www.youtube.com/@BMadCode)

### 進階主題

- **客製化代理** - 根據项目需求调整代理行為
- **擴充套件包** - 遊戲开发、創意寫作等專業領域支援
- **企業整合** - 大型團隊和企業環境的最佳實踐
- **效能優化** - 大型项目的擴展策略
...
- **开发效率提升 40%** - 减少重复工作和溝通成本
- **錯誤率降低 60%** - 內建品質檢查和测试策略
- **交付時間縮短 30%** - 标准化流程和自動化工具
...
*"BMad Method V6 不只是工具，更是开发團隊的超能力。讓 AI 处理重複工作，讓人類专注于創造。"*

*BMad Method 增強您的开发流程，而不是取代您的專業知識。*
